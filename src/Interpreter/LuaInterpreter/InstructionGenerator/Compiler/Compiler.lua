...
Compiler.Config = {
    stripSource = true,
    sourceName = "@protected.lua",
    protectConstants = false
}

function Compiler.configure(config)
    for key, value in pairs(config or {}) do
        if Compiler.Config[key] ~= nil then
...
function Compiler.compileHeader()
    local header = MAGIC
    header = header .. makeOneByte(VERSION)
    header = header .. makeOneByte(0)
    header = header .. makeOneByte(1)
    header = header .. makeOneByte(4)
    header = header .. makeOneByte(8)
    header = header .. makeOneByte(4)
    header = header .. makeOneByte(8)
    header = header .. makeOneByte(0)
    return header
...
function Compiler.compileFunction(proto)
    ...
    local sourceName = Compiler.Config.sourceName

    if not Compiler.Config.stripSource then
        sourceName = proto.sourceName or sourceName
    end

    functionProto = functionProto .. Compiler.compileString(sourceName)
    ...
    functionProto = functionProto .. Compiler.compileConstants(proto.constants or {})
    functionProto = functionProto .. Compiler.compileInt(0)
    functionProto = functionProto .. Compiler.compileInt(0)
    functionProto = functionProto .. Compiler.compileInt(0)
    functionProto = functionProto .. Compiler.compileInt(0)
...
function Compiler.compileString(str)
    local size = #str + 1
    local compiledString = makeEightBytes(size)
    compiledString = compiledString .. str
    compiledString = compiledString .. makeOneByte(0)
...
local function twos_complement(n, bits)
    if n < 0 then
        n = n + 1
...
function Compiler.compileFunctions()
end

function Compiler.compileDebug()
end
